...
function FriendCard(props: { data: IFriendData }) {
  ...
  let str = '';
  data.characteristic.map((el: string, idx: number) => {
    str += el + ' ';
  });

  const ENAME = data.ename;

  const tmp = data.desc.replace(/(?:\r\n|\r|\n)/g, '<br />');
  const imgSrc = `assets/img/${data.ename}/2d.svg`;

  const getTheme = () => {
    if (customTheme === 'haruDark') return haruDark;
    else if (customTheme === 'haruLight') return haruLight;
    else if (customTheme === 'toriDark') return toriDark;
    else if (customTheme === 'toriLight') return toriLight;
    else if (customTheme === 'gomiDark') return gomiDark;
    else return gomiLight;
  };
  ...
  const handleCurrFriendShip = () => {
    const isConfirm = confirm(`${data.name}을 선택하시겠어요?`);
    if (isConfirm) {
  ...
  const handleCharacteristic = () => {
    const CharacteristicArr = data.characteristic.map(
      (el: string, index: number) => {
        return (
          <Characteristic theme={getTheme()} key={index}>
            {el}
          </Characteristic>
        );
      },
